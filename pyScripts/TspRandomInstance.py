...
def main():
    ...
    # set the points to have range [0, ptRange)
    points = points * ptRange

    # check that all points are different
    i = 1
    while i < n:
        if all(np.isin(points[i], points[0:i-1])):
            points[i] = np.random.rand(2) * ptRange
            i -= 1
            break
        i += 1
...
def arg_parser() -> tuple[ int, str, str, int]:
    ...
    parser.add_argument('distance_type', choices=["EUC_2D","MAN_2D","MAX_2D", "CEIL_2D", "ATT"], help='Type of distance used. Supported types only.')
    parser.add_argument('-o', '--output_file', metavar='OUTPUT_FILENAME')
    parser.add_argument('-s', '--seed', metavar='SEED', type=int, help='Set random seed manually')
    ...
    if args.output_file == None:
        filename = "{0}-{1}-{2}.tsp".format(n, dist_type, seed)
    else:
