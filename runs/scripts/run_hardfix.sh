...
# solverFixedArgs="-m hardfix --metaInit nn --cplexInit vns --graspType almostbest --round "
# for i in ${!subDirs[@]}; do
#     for file in $inputDir/${subDirs[$i]}/*; do
#         for repeatCounter in $(seq 1 $nRuns); do
#             seed=$SRANDOM
#             outFullFilename=$outDir/$outFname\_$(basename ${file%.*})\_${tlims[$i]}s\_vnsInit\_$seed.log
...
solverFixedArgs="-m hardfix --cplexInit nn --round"
for i in ${!subDirs[@]}; do
    for file in $inputDir/${subDirs[$i]}/*; do
        for repeatCounter in $(seq 1 $nRuns); do
            seed=$SRANDOM
