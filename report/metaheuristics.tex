...
It's also worth mentioning the fact that heuristics like NN and EM are limited in discovering new solutions at each iteration, while metaheuristics usually start from an existing feasible solution, with the objective of improving it.
...
Another approach consists in scanning all possible swaps like before, and selecting the swap with has the lowest offset.
...
As opposed to the first method, the second procedure takes longer to find swaps, but the swap slected is the one that reduces the cost of the solution the most.
...
Each iteration of 2-Opt is of $O(n^2)$ complexity, while the number of iterations depends both on the quality of the starting solution.
...
As expected, the performance of the 3-Opt algorithm consistently surpasses that of 2-Opt.
...
Since 2-Opt is an external method we needed to figure out a way to prevent it to perform swaps which involve edges locked in the tenure.
...
We three different values which are compared in \figurename{ \ref{fig:tabuParmTune}}.
