...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
	\caption{Performance comparison between 2-Opt and 3-Opt} \label{fig:23OptPerformance}
\end{figure}
...
This is strictly dependent on our implementation of the Tabu Search method, since other implementations might use different edge locking mechanics as well as different local search method, therefore favoring different values. %for the tenure size.
...
\begin{figure}[htbp]
    \begin{algorithm}[H]
        \TitleOfAlgo{\textbf{Simulated Annealing}}
        \SetKwInOut{Input}{Input}
        \SetKwInOut{Output}{Output}
        \Input{Graph $G(V,E)$ \newline Cost function $c$ \newline A tour $T$ of $G$ \newline Starting temperature $k_s$ }
        \Output{A tour $T_{best}$ where $c(T_{best}) \leq c(T)$}
        \BlankLine
        $T_{best} \gets T$\\
        \While{time limit is not reached}{
            $k \gets k_s$ // temperature \\
            $noMove \gets $ False \\
            $m \gets $ NULL \\
            \While{$(k > k_{lower bound})$ AND $(!noMove)$}{
                $\sigma \gets $ False\\
                \For{$(i \in \{0,1,\dots, max\_tries\})$ AND $(!\sigma)$}{
                    \BlankLine
                    $m \gets $ swap chosen at random in $T$ \\
                    $\sigma \gets $ AcceptMove($T$, $m$)
                }
                \eIf{$\sigma$}{
                    PerformMove($T$, $m$) \\
                    %\lIf{$c(T) \leq c(T_{best})$}{$T_{best} \gets T$}
...
To properly explain this behavior we would require much more time compared to other algorithms, like NN or VNS, since not only there are more parameters to tune, but we would also need to consider the different quality criteria used.
...
In TSP case, the strong point of Genetic Algorithms are the complexity and enormity of the explorable solution space, which is a direct consecuence of the high randomness characteristic of the method. 
...
\begin{figure}[H]
    \begin{algorithm}[H]
        \TitleOfAlgo{\textbf{Genetic Algorithm}}
        \SetKwInOut{Input}{Input}
        \SetKwInOut{Output}{Output}
        \Input{Graph $G(V,E)$ \newline Cost function $c$ \newline Population size $s_p$ \newline Crossover amount $s_c$ \newline Mutation amount $s_m$ \newline Reintroduction amount $s_r$}
        \Output{A tour $T$ of $G$}
        \BlankLine

        $P \gets \{\}$ \\
        \For{$i \in \{0,1,\dots, s_p\}$}{
            \BlankLine
            $p_i \gets$ tour generated through permutations\\
            $P \gets P \cup {p_i}$
        }

        $T_{best} \gets$ best solution in $P$\\

        \While{time limit is not reached}{
            $C \gets \{\}$ \\
            \For{$i \in \{0,1,\dots, s_c\}$}{
                \BlankLine
                $j,k \in \{0,1,\dots, s_p\}$, $j \neq k$ \\
                $c_i \gets$ EdgeRecombinationCrossover($P[j]$, $P[k]$)\\
                $C \gets C \cup {c_i}$
            }
            
            $M \gets \{\}$ \\
            \For{$i \in \{0,1,\dots, s_m\}$}{
                \BlankLine
                $j \gets$ random $\in \{0,1,\dots, s_p\}$ \\
                $m_i \gets$ Mutate($P[j]$) \\
                $M \gets M \cup {m_i}$
            }

            $R \gets \{\}$ \\
            \For{$i \in \{0,1,\dots, s_r\}$}{
                \BlankLine
                $r_i \gets$ tour generated through permutations \\
                $R \gets R \cup {r_i}$
            }

            $T \gets$ best solution in $P \cup C \cup M \cup R$ \\
            \lIf{$c(T) \leq c(T_{best})$}{$T_{best} \gets T$}

            $P \gets$ NaturalSelection($P \cup C \cup M \cup R$)
        }
        \Return $T_{best}$
    \end{algorithm}
    \caption{Pseudocode of GA} \label{fig:geneticPseudocode}
\end{figure}
...
As \figurename{ \ref{fig:geneticParmTune}} shows, there is a clear advantage of the first configuration in across the board, however, as it was for some of the previous cases this does not prove that the first configuration is the better one.
...
In fact the reason for its dominance in the graph is the ease of computation that this configuration has compared to the other ones.
\figurename{ \ref{fig:geneticIters}} shows this very clearly, with the first configuration running as fast as five times faster than the other ones.
However speed is not the only thing that matters: configuration 2 is the second fastest, however it is also the one that has the worst preformance overall.
This serves to prove that more time would be needed to properly find a “best” configuration, since there are too many parameters in play here.
...
Finally, \figurename{ \ref{fig:geneticCost}} shows how well the solutions found by GA compare to the optimal solutions.
Seeing how big is the scale on the \textit{x} axis in this graph we can clearly state that, in this project, GA was the worst performing algorithm.

\section{Comparison}

To conclude this chapter lets put all of the metaheuristics shown in a final comparsion.
...
As \figurename{ \ref{fig:metacmp}} illustrates, VNS emerged as the best metaheuristic identified in this project.
GA is not shown in the graph because its performance was significantly lower than that of the other methods, even though we used a larger time frame during its testing in order to obtain reasonable results.
Including it would have either caused it to disappear from the graph or required a much greater zoom, which would have made comparing the other algorithms more challenging.
